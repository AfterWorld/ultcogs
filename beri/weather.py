...
WEATHER_STATES: dict[str, dict] = {
    "calm_seas": {
        ...
        "weight": 30,                   # how often it's picked
        "colour": 0x3498db,
        "modifiers": {
            "fishing_multiplier":       1.0,
            "work_multiplier":          1.0,
            "crime_success_delta":      0.0,
            "plunder_multiplier":       1.0,
            "heat_decay_rate":          1.5,    # heat decays faster
            "heat_threshold_modifier":  0,
        },
        "transitions": {                # what weathers can follow this one
            "weights": {
                "storm":            20,
                "marine_blockade":  15,
                "new_world_current":10,
        ...
        "announce": {
            ...
            "end": [
                "The calm is ending. Something is coming.",
            ],
        },
    },
    ...
    "storm": {
        ...
        "modifiers": {
            "fishing_multiplier":       0.5,
            "work_multiplier":          0.75,
            "crime_success_delta":      0.15,   # chaos helps criminals
            "plunder_multiplier":       1.3,
            "duel_payout_multiplier":   1.5,
            "heat_decay_rate":          2.0,    # heat burns off fast
            "heat_threshold_modifier":  -20,    # lower threshold to trigger events
            "legendary_catch_weight":   8,      # rare fish surface
        },
        "transitions": {
            "weights": {
                "calm_seas":        40,
        ...
        "announce": {
            "start": [
                "⛈️ **STORM WARNING** — A violent storm is forming across the Grand Line.\n"
                "Marine patrols have pulled back. Criminal activity is spiking.\n"
                "Fishing yields halved. Crime success rates rising.",
                "The sky has turned black. Whatever you were planning — now's the time.",
            ],
            "end": [
                "The storm is passing. Marine vessels are resuming patrol routes.",
    ...
    "marine_blockade": {
        ...
        "modifiers": {
            "fishing_multiplier":       1.0,
            "work_multiplier":          1.4,    # legit work more rewarding
            "crime_success_delta":      -0.25,
            "plunder_multiplier":       0.4,
            "plunder_disabled":         False,  # not disabled, just punishing
            "rob_success_delta":        -0.20,
            "heat_decay_rate":          0.3,    # heat barely decays
            "heat_threshold_modifier":  +30,    # events trigger easier
        },
        "transitions": {
            "weights": {
                "calm_seas":        50,
        ...
        "announce": {
            "start": [
                "⚓ **MARINE BROADCAST** — Fleet Admiral has ordered a full blockade.\n"
                "All vessels subject to inspection. Legitimate workers are compensated.\n"
                "**Criminal success rates significantly reduced.**",
                "The Marines have mobilized. Every port is being watched.",
            ],
            "end": [
                "The Marine blockade is lifting. Ships are free to move.",
    ...
    "new_world_current": {
        ...
        "modifiers": {
            ...
            "work_multiplier":          1.2,
            "legendary_catch_weight":   25,
            "rare_catch_weight":        15,
            "heat_decay_rate":          1.0,
            "heat_threshold_modifier":  0,
            "beri_drop_bonus":          0.10,   # all income +10%
        },
        "transitions": {
            "weights": {
                "calm_seas":        60,
        ...
        "announce": {
            "start": [
                "🌀 **NEW WORLD CURRENT DETECTED** — Rare creatures are surfacing.\n"
                "Fishing yields massively boosted. Legendary catches possible.\n"
                "**This window won't last long.**",
            ],
            "end": [
                "The current has passed. The deep sea returns to silence.",
    ...
    "fog_of_war": {
        ...
        "modifiers": {
            ...
            "work_multiplier":          0.9,
            "crime_success_delta":      0.10,
            "intel_quality_penalty":    0.5,    # intel reports less accurate
            "duel_outcome_randomness":  0.15,   # upsets more likely
            "heat_decay_rate":          1.2,
            "heat_threshold_modifier":  -10,
        },
        "transitions": {
        ...
        "announce": {
            "start": [
                "🌫️ A thick fog has settled over the Grand Line.\n"
                "Navigation is difficult. Duel outcomes are less predictable.\n"
                "Intel reports are unreliable. Move carefully.",
            ],
            "end": [
                "The fog is clearing. Visibility returning to normal.",
            ],
    ...
    "red_line_eclipse": {
        "label": "🔴 Red Line Eclipse",
        "short": "eclipse",
        "duration_range": (1, 1),       # always exactly 1 hour
        "weight": 3,                    # very rare
        "colour": 0xe74c3c,
        "modifiers": {
            "all_multiplier":           2.0,    # everything doubled
            "fishing_multiplier":       3.0,
            "heat_decay_rate":          3.0,
            "heat_threshold_modifier":  -40,
            "legendary_catch_weight":   50,
            "fruit_resonance_active":   True,
        },
        "transitions": {
            "weights": {
                "calm_seas":        70,
    ...
    "yonko_territory": {
        ...
        "modifiers": {
            ...
            "crime_success_delta":      -0.10,
            "plunder_multiplier":       0.7,
            "duel_payout_multiplier":   2.0,    # fights here pay well
            "heat_threshold_modifier":  +15,
            "heat_decay_rate":          0.7,
            "tribute_active":           True,   # 2% of income goes to "Yonko"
        },
        "transitions": {
            "weights": {
                "calm_seas":        40,
        ...
        "announce": {
            "start": [
                "👑 **YONKO TERRITORY** — You've drifted into Emperor's waters.\n"
                "Duels pay double. Work is rewarded. But crossing a Yonko has consequences.\n"
                "**2% tribute is automatically collected from all Beri earned.**",
            ],
            "end": [
                "You've left Yonko territory. The Emperor's influence fades.",
...
def _next_weather(current: str) -> str:
    """
    Pick the next weather state using the current state's transition weights.
    Falls back to uniform random if current state not found.
    """
    state = WEATHER_STATES.get(current)
    if state is None:
        return random.choices(
    ...
    transitions = state["transitions"]["weights"]
    options = list(transitions.keys())
    weights = [transitions[o] for o in options]
    return random.choices(options, weights=weights, k=1)[0]